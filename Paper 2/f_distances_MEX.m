...
function results=f_distances_MEX(spikes,d_para,f_para,N)
...
max_memo_init=10000000000;      % memory management, should be big enough to hold the basic matrices but small enough to not run out of memory
...
if mod(s_para.plot_mode,2)>0
    ...
    if isfield(d_para,'markers2')
        for mac=1:length(d_para.markers2)
            line(d_para.mspikesarkers2(mac)*ones(1,2),sum(f_para.subplot_size)-[0.05 1.05],'Color','r','LineStyle','-','LineWidth',2)
        end
    end
