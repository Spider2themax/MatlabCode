...
%stims = [5 10 15 20 25 30 35 40 45 50];
            ...
            for con = 1:2,
                ...
                if N > numNeuronsToKeep,
                    ...
                    for iter = 1:25,
                        ...
                        data(fileIndex, j, 2*(age-1) + con) = data(fileIndex, j, 2*(age-1) + con) + results.overall_dissimilarities;

                    end
                    
                    data(fileIndex, j, 2*(age-1) + con) = data(fileIndex, j, 2*(age-1) + con)/25;

                else

                    results=f_distances_MEX(spikes,d_para,f_para);

                    data(fileIndex, j, 2*(age-1) + con) = results.overall_dissimilarities(1);
