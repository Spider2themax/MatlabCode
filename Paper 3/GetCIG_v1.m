function [CIG] = GetCIG_v1(geo, Conn, curL, node, dim)
...
for i = 1:n-1, 
    for j = 1:n-1, % Iterate through all nodes
        ...
        if length(temp) > 1,
            ...
            CIG = CIG + sum(sum(Conn(temp, temp)))/(length(temp)^2 -length(temp));  % This is a great ass line of code if I do say so myself
